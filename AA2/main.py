...
class EmailSenderApp(Tk):
    def __init__(self):
        ...
        Label(self.frame, text="Subject:").grid(column=0, row=6, sticky="w")
        self.subject_entry = Entry(self.frame)
        self.subject_entry.grid(column=1, row=6, padx=10, pady=5, columnspan=2)

        Label(self.frame, text="Message:").grid(column=0, row=7, sticky="w")
        self.message_body_text = Text(self.frame, width=40, height=10)
        self.message_body_text.grid(
            column=1, row=7, padx=10, pady=5, columnspan=2)

        self.send_button = Button(
            self.frame, text="Send Email", command=self.send_email)
        self.send_button.grid(column=1, row=8, padx=10, pady=10, columnspan=2)
